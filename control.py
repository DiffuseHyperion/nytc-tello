...
class FrontEnd(object):
  ...
  def __init__(self):
    ...
    self.screen = pygame.display.set_mode([960, 660])
  ...
  def run(self):
    ...
    while not should_stop:
      ...
      cv2.putText(frame, text, (5, 660 - 5),
  ...
  def keydown(self, key):
      ...
      if key == pygame.K_UP:  # set forward velocity
        ...
        photo_frame = self.tello.get_frame_read()
        cv2.imwrite(f"picture-{time.time()}.png", cv2.cvtColor(photo_frame.frame, cv2.COLOR_BGR2RGB))
